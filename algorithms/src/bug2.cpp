...
private:
    ...
    rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr publisher_straight_line_;
    ...
    std::vector<VertexDijkstra> straight_line_path_points;
        
    std::unordered_set<std::pair<float, float>, PairHash> obstaclesVertices;
    std::unordered_map<std::pair<float, float>, int, PairHash> straight_line_points;
    std::vector<std::pair<std::pair<float, float>, int>> next_point;
    ...
    std::vector<std::pair<float, float>> bug1(std::pair<float, float> initial_pose, std::pair<float, float> goal_pose)
    {
        ...
        straight_line_to_goal(initial_pose, goal_pose);
        ...
        int l = 0;
        float first_point_distance = std::numeric_limits<float>::infinity();
        ...
        do
        {
            ...
            std::pair<float, float> first_point;
                ...
                for (const auto& current_point : adjacency_list[actual_obstacle])
                {
                    for (int a = 0; a < 8; a++)  
                    {
                        ...
                        if (adjacency_list[actual_obstacle].find(neighbor_tuple) != adjacency_list[actual_obstacle].end() && obstaclesVertices.find(neighbor_tuple) == obstaclesVertices.end() && visited_points.find(neighbor_tuple) == visited_points.end() && obstaclesVertices.find(current_point) != obstaclesVertices.end() && visited_obstacles.find(current_point) == visited_obstacles.end())
                        {
                            ...
                            float temp_distance = heuristic(actual_point, goal_pose);

                            if(temp_distance < distance)
                            {
                                distance = temp_distance;
                                closer_point = actual_point;
                            }

                        
                            if(straight_line_points.find(actual_point) != straight_line_points.end() && distance < first_point_distance)
                            {
                                
                                actual_obstacle = actual_point;
                                break_all = true;
                                break;
                            }
                        ...
                        else if(obstaclesVertices.find(neighbor_tuple) == obstaclesVertices.end() && i > 10)
                        {
                            ...
                            if(straight_line_points.find(neighbor_tuple) != straight_line_points.end() && distance < first_point_distance)
                            {
                                intersection_point = neighbor_tuple;
                                visited_points.insert(neighbor_tuple);
                                actual_obstacle = neighbor_tuple;
                                break_all = true;
                                break;
                            }
                        }
                        ...
                        for (int a = 0; a < 8; a++)  
                        {
                            ...
                            && obstaclesVertices.find(current_point) != obstaclesVertices.end() && visited_obstacles.find(current_point) == visited_obstacles.end())
                            {
                                ...
                                if(straight_line_points.find(actual_point) != straight_line_points.end() && distance < first_point_distance)
                                {
                                    ...
                                    intersection_point = actual_point;
                                    actual_obstacle = actual_point;
                                    break_all = true;
                                    break;
                                }
            ...
            for (const auto& current_point : visited_obstacles)
            {
                for (int a = 0; a < 8; a++)  
                {
                    ...
                    if (obstaclesVertices.find(neighbor_tuple) == obstaclesVertices.end() && visited_points.find(neighbor_tuple) == visited_points.end() && straight_line_points.find(neighbor_tuple) == straight_line_points.end())
                    {
                        actual_point = neighbor_tuple;
                        visited_points.insert(neighbor_tuple);
                        std::this_thread::sleep_for(std::chrono::milliseconds(2));
            ...
            std::unordered_map<std::pair<float,float>, std::pair<float,float>, PairHash> parent;
            ...
            auto new_path = run_a_star_to_find_path_visited_points(first_point, intersection_point);
            path.insert(path.end(), new_path.begin(), new_path.end());
           
         
          

            int index = straight_line_points[actual_obstacle];
            std::pair<float, float> temp_2;

            bool found_obstacle = false;

            for(size_t f = index; f < next_point.size(); f++)
            {
                if(next_point[f].second == 1)
                {
                    actual_obstacle = temp_2; 
                    break;
                }
                else if(next_point[f].second == 0)
                {
                    path.push_back(next_point[f].first);
                    temp_2 = next_point[f].first;
                }
                else if(next_point[f].second == 2)
                {
            ...
            first_point_distance = heuristic(actual_obstacle, goal_pose);
    ...
    std::vector<std::pair<float, float>> run_a_star_to_find_path_visited_points(std::pair<float, float> start_tuple, std::pair<float, float> goal_tuple) 
    {
    ...
    void straight_line_to_goal(std::pair<float, float> start_tuple, std::pair<float, float> goal_tuple)
    {
        int k = 0; 
        std::vector<std::pair<float, float>> path;
        ...
        while (t < distance && obstacleFound == false) 
        {
            ...
            straight_line_points[neighbor_tuple] = k;
            path.push_back(neighbor_tuple);
            ...
            if(obstaclesVertices.find(neighbor_tuple) == obstaclesVertices.end())
            {
                std::pair<std::pair<float, float>, int> toma = std::make_pair(neighbor_tuple, 0);

                next_point.push_back(toma);
            }
            else
            {
                std::pair<std::pair<float, float>, int> toma = std::make_pair(neighbor_tuple, 1);
        ...
        std::pair<std::pair<float, float>, int> toma = std::make_pair(goal_tuple, 2);
        next_point.push_back(toma);
    
    

        straight_line_path_points.clear();
        ...
        for (size_t i = 0; i < path.size(); i++) 
        {
            ...
            straight_line_path_points.push_back(vertex);
        }

        publisher_straight_line();
    ...
    */
    ...
    void publisher_straight_line()
    {
        nav_msgs::msg::Path path_msg;
        path_msg.header.stamp = this->now();
        path_msg.header.frame_id = "map";

        for (const auto& vertex : straight_line_path_points)
        {
            geometry_msgs::msg::PoseStamped pose_stamped;
            pose_stamped.header.stamp = this->now();
            pose_stamped.header.frame_id = "map";
        ...
        publisher_straight_line_->publish(path_msg);
    }
...
public:
     ...
     : Node("Bug_2")
    {
        ...
        publisher_straight_line_ = this->create_publisher<nav_msgs::msg::Path>("/straight_line", 10);
